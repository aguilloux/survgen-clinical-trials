...
def run(generator_name, target_epsilon):
    ...
    # Always use 100% of the control data (the "3/3" equivalent)
    perc_control = 1.0
    d = 2
    ...
    name_config = "traincontrol_" + dataset_name + "_aug_Ncontrol{}%3_{}".format((d + 1), eps_tag)

    generators_dict = {"HI-VAE_weibull_DP": surv_hivae,
    ...
    source_name_config = "best_params_traincontrol_" + dataset_name + "_aug_Ncontrol3%3_ntrials150_" + optuna_version_name
    source_best_params_file = None
    for f in os.listdir(parent_path + "/dataset/" + dataset_name + "/optuna_results"):
        if f.endswith(base_generator_name + ".json") and (source_name_config in f):
            source_best_params_file = f
    if source_best_params_file is None:
        raise FileNotFoundError("Could not find non-DP best params file matching '{}...{}.json'.".format(source_name_config, base_generator_name))
    ...
    for i in range(len(feat_types_dict)):
        if feat_types_dict_ext[i]['name'] == "survcens":
            if generator_name in ["HI-VAE_weibull_DP"]:
