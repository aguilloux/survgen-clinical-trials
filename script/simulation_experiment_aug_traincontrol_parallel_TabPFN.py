...
sys.path.append(str(module_path))
import surv_hivae, surv_gan, surv_vae, surv_tabpfn
...
def run(MC_id, gen_id=None):
    ...
    # writes a per-generator results file. Recombine the 5 files per MC block with
    # `script/merge_TabPFN_results.py`.
    if gen_id is not None:
        generators_sel = [generators_sel[gen_id]]
        result_tag = generators_sel[0].replace("/", "-")
    else:
        result_tag = "TabPFN"
    ...
    for d, perc_control in enumerate(list_n_samples_control):
        ...
        for generator_name in generators_sel:
            if "TabPFN" not in generator_name:
    ...
    results.to_csv(f"{parent_path}/dataset/{dataset_name}/results_{result_tag}_n_samples_{n_samples}_n_features_bytype_{n_features_bytype}_MC_{MC_init}to{MC_final}.csv")
