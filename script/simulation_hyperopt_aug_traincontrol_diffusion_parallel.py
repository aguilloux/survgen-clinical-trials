...
def run(generator_name):
    ...
    for d, perc_control in enumerate(list_n_samples_control):
        ...
        study_name = parent_path + "/dataset/" + dataset_name + "/optuna_results/optuna_study_{}_ntrials{}_{}_{}".format(name_config, n_trials, metric_optuna[0], generator_name)
        best_params_file = parent_path + "/dataset/" + dataset_name + "/optuna_results/best_params_{}_ntrials{}_{}_{}.json".format(name_config, n_trials, metric_optuna[0], generator_name)
        diffusion = "_diffusion" in generator_name
        diffusion_pre_params = None
        if diffusion:
            best_params_file_wo_diffusion = parent_path + "/dataset/" + dataset_name + "/optuna_results/best_params_{}_ntrials{}_{}_{}.json".format(name_config, n_trials, metric_optuna[0], generator_name.replace("_diffusion", ""))
            with open(best_params_file_wo_diffusion, "r") as f:
                diffusion_pre_params = json.load(f)
