...
def run(dataset_name, generators_sel):
    ...
    for generator_name in generators_sel:
        ...
        #         best_param_file = f
        best_param_file = os.path.join(best_param_dir, "best_params_{}_ntrials{}_{}_{}.json".format(name_config, n_trials, optuna_version_name, generator_name))
    ...
    if not os.path.exists(parent_path + "/dataset/" + dataset_name + f"/metric_results_{optuna_version_name}"):
        os.makedirs(parent_path + "/dataset/" + dataset_name + f"/metric_results_{optuna_version_name}")
    ...
    general_scores_df.to_csv(parent_path + "/dataset/" + dataset_name + f'/metric_results_{optuna_version_name}/traincontrol_general_scores_df.csv', index=False)
    ...
    replicability_scores_df.to_csv(parent_path + "/dataset/" + dataset_name + f'/metric_results_{optuna_version_name}/traincontrol_replicability_scores_df.csv', index=False)
    ...
    err_df = pd.DataFrame({"midpoints" : midpoints,
                          ...
                          "label" : label,
                          "colors" : colors_})
    err_df.to_csv(parent_path + "/dataset/" + dataset_name + f'/metric_results_{optuna_version_name}/traincontrol_error_df.csv', index=False)
