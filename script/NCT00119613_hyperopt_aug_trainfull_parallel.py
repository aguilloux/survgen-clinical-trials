...
def run(generator_name):
    ...
    for d, perc_control in enumerate(list_n_samples_control):
        ...
        name_config = "trainfull_" + dataset_name + "_aug_Ncontrol{}%3".format((d+1))
        ...
        if generator_name in ["HI-VAE_lognormal", "HI-VAE_weibull", "HI-VAE_piecewise", "HI-VAE_weibull_prior", "HI-VAE_piecewise_prior"]:
            feat_types_dict_ext = feat_types_dict_full_ext.copy()
            ...
            condition = {'var': 'treatment_0', 'value': 1.0, 'n_samples': n_generated_samples_control}  # Condition on the control group
            ...
            condition = {'var': 'treatment', 'value': 0.0, 'n_samples': n_generated_samples_control}
            ...
            indices = torch.cat((torch.arange(0, n_samples_control), torch.randint(0, n_samples_control, (n_generated_samples_control - n_samples_control,))))
            cond_gen = SurvivalAnalysisDataLoader(df_init_control_ext.loc[indices], target_column="censor", time_to_event_column="time")[["censor", "treatment"]]
            best_params, study = generators_dict[generator_name].optuna_hyperparameter_search(data_init_full_ext, 
                                                                                            columns=fnames, 
                                                                                            target_column="censor", 
                                                                                            time_to_event_column="time", 
