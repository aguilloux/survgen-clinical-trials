...
from metrics import fit_cox_model, general_metrics, replicability_ext
...
def run(dataset_name, generators_sel):
    ...
    for d, perc_control in enumerate(list_n_samples_control):
        ...
        n_generated_samples_control = df_init_control_encoded_ext.shape[0]
        n_samples_control = int(perc_control * n_generated_samples_control)
        print("n_samples_control:", n_samples_control)
        df_init_control_encoded_ext = df_init_control_encoded_ext.iloc[:n_samples_control]
        ...
        df_init_full_encoded_ext, feat_types_dict_full_ext, miss_mask_full, true_miss_mask_full, _ = data_processing.read_data(data_file_full_ext, 
                                                                                                                    ...
                                                                                                                    miss_file, true_miss_file)
        df_init_control_encoded_ext_sel = df_init_full_encoded_ext[df_init_full_encoded_ext.treatment_0 == 1.]
        df_init_control_encoded_ext_sel = df_init_control_encoded_ext_sel.iloc[:n_samples_control]
        df_init_treated_encoded_ext_sel = df_init_full_encoded_ext[df_init_full_encoded_ext.treatment_0 == 0.]
        df_init_full_encoded_ext = pd.concat([df_init_control_encoded_ext_sel, df_init_treated_encoded_ext_sel], axis=0, ignore_index=True)
        ...
        data_init_treated_encoded_ext =  torch.from_numpy(df_init_treated_encoded_ext_sel.values)
        data_init_treated_ext = data_processing.discrete_variables_transformation(data_init_treated_encoded_ext, feat_types_dict_full_ext)
        df_init_treated_ext = pd.DataFrame(data_init_treated_ext.numpy(), columns=fnames)
        ...
        for generator_name in generators_sel:
            ...
            for f in os.listdir(best_param_dir):
                if (f.endswith(generator_name + '.json') & ("trainfull_" + name_config  + "_aug_Ncontrol{}%3".format((d+1)) in f)):
                    best_param_file = f
        ...
        for generator_name in generators_sel:
            ...
            if generator_name in ["HI-VAE_lognormal", "HI-VAE_weibull", "HI-VAE_piecewise", "HI-VAE_weibull_prior", "HI-VAE_piecewise_prior"]:
                ...
                feat_types_dict_ext = adjust_feat_types_for_generator(generator_name, feat_types_dict_full_ext)
                condition = {'var': 'treatment_0', 'value': 1.0, 'n_samples': df_init_control_ext.shape[0]}  # Condition on the control group
                data_gen_control = generators_dict[generator_name].run(df_init_full_encoded_ext, miss_mask_full, 
                ...
                condition = {'var': 'treatment', 'value': 0.0, 'n_samples': df_init_control_ext.shape[0]}
                data_gen_control = generators_dict[generator_name].run(data_init_full_ext, columns=fnames, 
            ...
            for i in range(n_generated_dataset):
                ...
                data_syn.append(pd.concat([df_init_treated_ext, df_gen_control], ignore_index=True))
        ...
        general_scores = []
        for generator_name in generators_sel:
            general_scores.append(general_metrics(df_init_control_ext, df_gen_control_dict[generator_name], generator_name))
        general_scores_df = pd.concat(general_scores)
        general_scores_df.to_csv(parent_path + "/dataset/" + dataset_name + '/metric_results/trainfull_aug_Ncontrol{}%3_general_scores_df.csv'.format((d+1)), index=False)
